...
loger()
{
	...
	if [ "$logt" = "i" -a -z "$1" ];then
		...
		adb $ss pull /sdcard/logs/ ./$n"_"$pkg/logs
	elif [ "$1" = "1" ];then
		adb $ss pull /logs/ ./system/logs
		adb $ss pull /sdcard/logs/ ./system/logs
...
preboot()
{
	...
	sleep 10
	echo "`date`: Unlocking..."
	adb $ss shell input swipe 946 374 1079 393
}

...
mt()
{
	...
	[ -z "$1" ] && n=100
	echo "`date`: ############\nA call B(PUT),\n$n times.\n###########"
	echo "`date`: Please input number A:"
	read na
	echo "`date`: Please input number B:"
	read nb
	...
	while [ $i -le $n ]; do
		...
		while true;do
			...
			#(userdebug)rb="`adb -s $sb logcat -b radio -d | grep "onVoiceCallEnded"`"
			rb="`adb -s $sb logcat -b radio -d | grep "onDisconnect: cause=NORMAL"`"
			sleep 1
			...
			if [ $dur -eq 30 ];then	#time out
				...
				cleanup "-s" "$sb"
				cleanup "-s" "$sa"
				adb $sb shell input motionevent 150 360 down
				adb $4b shell input motionevent 150 360 up
				sleep 3
				doom=1
				break
			fi
...
cleanup()
{
	...
	adb $1 $2 shell rm -r /data/logs/aplog.[0-9][0-9] > /dev/null
	adb $1 $2 shell rm -r /data/logs/aplog.[456789] > /dev/null
	adb $1 $2 shell rm -r /data/logs/bplog.[2-9] > /dev/null

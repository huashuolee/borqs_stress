...
mt()
{
	...
	while [ $i -le $n ]; do
		...
		#adb -s $sa shell am start -a android.intent.action.CALL -d tel:$nb --ez "com.pekall.phone.extra.DSDS_CALL_FROM_SLOT_2" true
		adb -s $sa shell am start com.android.contacts/.activities.DialtactsActivity
		sleep 1
		adb -s $sa shell input text $nb
		sleep 1
		move 200 450
		#adb -s $sa shell input keyevent 5
		sleep 1

...
verification()
{
	...
	adb $sa shell am start -a android.intent.action.SENDTO -d sms:$num -n com.android.mms/.ui.ComposeMessageActivity -W
	sleep 1
	adb $sa shell input text testABC1234
	sleep 1
	...
	adb $sa shell input keyevent 23
	while true;do
		ra=`adb $sa shell logcat -d -b radio -v time | grep Received`
		rb=`adb $sb shell logcat -d -b radio -v time | grep Received`
		if [ -n "$ra"  ];then
			tmp="$sa"
			sa="$sb"
			sb="$tmp"
...
mms()
{
	...
	while [ $n -le 100 ];do
		...
		while  [ $dur -le 900 ]; do
			sleep 1
			r="`adb $sb shell logcat -d -b events | grep "com.android.mms" | grep "notification_sound"`"
			if [ -n "$r"  ];then
...
sms()
{
	...
	while [ $n -le $1 ];do
		...
		while  [ $dur -le 60 ]; do
			sleep 1
			r=`adb $sb shell logcat -d -b radio -v time | grep Received`
			if [ -n "$r"  ];then
